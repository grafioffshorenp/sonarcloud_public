  ...
  <div class="relative mx-auto max-w-2xl py-24 sm:py-32 lg:max-w-4xl">
    ...
    <figure class="grid grid-cols-1 items-center gap-x-6 gap-y-8 lg:gap-x-10">
      <div class="relative col-span-2 lg:col-start-1 lg:row-start-2">
        ...
        <blockquote class="text-xl font-semibold leading-8 text-gray-900 sm:text-2xl sm:leading-9" v-html="block.quote">
        </blockquote>
      </div>
      <div class="col-end-1 w-16 lg:row-span-4 lg:w-72">
        <img class="rounded-xl bg-indigo-50 lg:rounded-3xl" :src="block.avatar" alt="" />
      </div>
      <figcaption class="text-base lg:col-start-1 lg:row-start-3">
        <div class="font-semibold text-gray-900">{{block.quotee}}</div>
        <div class="mt-1 text-gray-500">{{ block.function }}</div>
      </figcaption>
    </figure>
  </div>
</template>
